...
const viewProblem = function(idx) { return slideWrapper([PuraRuntime.elem('h1')([])([PuraRuntime.text("課題：実行時の脆弱性")]), PuraRuntime.elem('p')([PuraRuntime.attr('class')("subtitle")])([PuraRuntime.text("動的型付けが引き起こす静かなる失敗")]), PuraRuntime.elem('div')([PuraRuntime.attr('class')("code-block")])([line([kw("function"), norm(" calculateTotal(cart) {")]), line([norm("  "), kw("let"), norm(" total "), kw("="), norm(" 0;")]), line([norm("  "), kw("for"), norm(" ("), kw("let"), norm(" item "), kw("of"), norm(" cart.items) {")]), line([norm("    "), cmt("// バグ1: APIからの値が文字列 \"10\" の場合")]), line([norm("    "), cmt("// 加算ではなく文字列連結になる: \"010\"")]), line([norm("    total "), kw("+="), norm(" item.price;")]), line([norm("    "), cmt("// バグ2: taxが未定義(undefined)だとNaNになる")]), line([norm("    total "), kw("+="), norm(" item.tax;")]), line([norm("  }")]), line([norm("  "), kw("return"), norm(" total.toFixed(2); "), cmt("// NaNならクラッシュ")]), line([norm("}")])]), PuraRuntime.elem('p')([PuraRuntime.attr('class')("fragment visible")])([PuraRuntime.text("文法的に正しくても、本番環境でクラッシュします。")])]); };
