...
const viewProblem = function(idx) { return slideWrapper([PuraRuntime.elem('h1')([])([PuraRuntime.text("Challenge: Runtime Instability")]), PuraRuntime.elem('p')([PuraRuntime.attr('class')("subtitle")])([PuraRuntime.text("Dynamic typing creates silent failures.")]), PuraRuntime.elem('div')([PuraRuntime.attr('class')("code-block")])([line([kw("function"), norm(" calculateTotal(cart) {")]), line([norm("  "), kw("let"), norm(" total "), kw("="), norm(" 0;")]), line([norm("  "), kw("for"), norm(" ("), kw("let"), norm(" item "), kw("of"), norm(" cart.items) {")]), line([norm("    "), cmt("// BUG 1: 'price' is a string from API (\"10\")")]), line([norm("    "), cmt("// Result is string concatenation: \"010\"")]), line([norm("    total "), kw("+="), norm(" item.price;")]), line([norm("    "), cmt("// BUG 2: If item.tax is missing, it adds undefined (NaN)")]), line([norm("    total "), kw("+="), norm(" item.tax;")]), line([norm("  }")]), line([norm("  "), kw("return"), norm(" total.toFixed(2); "), cmt("// Crashes if NaN")]), line([norm("}")])]), PuraRuntime.elem('p')([PuraRuntime.attr('class')("fragment visible")])([PuraRuntime.text("Logic errors that pass syntax checks but crash production.")])]); };
