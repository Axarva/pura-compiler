{-# LANGUAGE LambdaCase #-} -- Useful for pattern matching in do blocks
...
import qualified Data.Map as Map -- For parseProgram merging


-- Parser monad type
type Parser = Parsec Void [L.Token]

-- Helper for chainr1 (right-associative binary operator)
...
parseArrowType = chainr1' parseBasicType (do _ <- single L.TokRightArrow; return TArr)

-- Parses atomic or parenthesized types
parseBasicType :: Parser Type
parseBasicType =
      (do L.TokIdentifier "Int"    <- satisfy isIdentifier; return TInt)
  <|> (do L.TokIdentifier "String" <- satisfy isIdentifier; return TString)
  <|> (do L.TokIdentifier "Bool"   <- satisfy isIdentifier; return TBool)
  <|> (do L.TokIdentifier "Unit"   <- satisfy isIdentifier; return TUnit)
...
parseListType = do
  L.TokIdentifier "List" <- satisfy isIdentifier -- Expects the keyword "List"
...
-- Helper for chaining left-associative binary operators
...
--------------------------------------------------------------------------------
-- 7. Program Parsing (Orchestrates top-level declarations)
--------------------------------------------------------------------------------
...
-- Parses any top-level declaration (either a type signature or a function definition)
...
-- Simple helper to find duplicates in a list
